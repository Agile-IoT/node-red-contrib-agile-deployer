...
module.exports = function(RED) {
    ...
    const d = require('debug')('cloud-link-switch')
    const request=require('request');
    const os = require('os');
    const perf = require('./perf-values')

    function CloudLinkSwitch(config) {
        ...
        node.on('input', function(msg) {
            ...
            getValues(config.dummy, msg).then( result => {
                ...
                d(`cloud-link-switch.on: error=${err}`)
    ...
    RED.nodes.registerType("cloud-link-switch", CloudLinkSwitch);
}
